...
{
    std::cout << "Task " << taskId << " marked as private" << std::endl;
    isPrivate = true;
    return true;
}
...
{
    std::cout << "Task " << taskId << " marked as public" << std::endl;
    isPrivate = false;
    return true;
}
...
{
    std::cout << "Creating tag for task " << taskId << " with name: " << tagName << std::endl;
...
{
    std::cout << "Deleting tag " << tag->tagName << " from task " << taskId << std::endl;
...
{
    std::cout << "Attaching file " << file->filePath << " to task " << taskId << std::endl;
...
{
    std::cout << "Removing file " << file->filePath << " from task " << taskId << std::endl;
...
{
    std::cout << "Assigning tag " << tag->tagName << " to task " << taskId << std::endl;
    tags.push_back(tag);
    return true;
}
...
{
    std::cout << "Removing tag " << tag->tagName << " from task " << taskId << std::endl;
    auto it = std::find(tags.begin(), tags.end(), tag);
    if (it != tags.end())
    {
        tags.erase(it);
...
{
    std::cout << "Task ID: " << taskId << ", Title: " << title << ", Description: " << description
        << ", Due Date: " << asctime(&dueDate) << ", Status: " << status << ", Private: " << isPrivate
        << std::endl;
}
